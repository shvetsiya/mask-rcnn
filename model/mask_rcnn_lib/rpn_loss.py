...
def weighted_binary_cross_entropy_with_logits(logits, labels, weights):
    ...
    loss = weights * (
        logits.clamp(min=0) - logits * labels + torch.log(1 + torch.exp(-logits.abs())))
...
def weighted_smooth_l1(predicts, targets, weights, sigma=3.0):
    ...
    loss = weights * (smooth_l1_option1 * smooth_l1_signs + smooth_l1_option2 *
                      (1 - smooth_l1_signs))
...
def rpn_loss(logits_flat,
    ...
    target_weights = target_weights.view((batch_size, num_windows, 1)).expand(
        (batch_size, num_windows, 4)).contiguous()
