...
def make_one_rcnn_target(cfg, proposal, truth_box, truth_label):
    ...
    if fg_inds_length > 0 and bg_inds_length > 0:
        ...
        fg_inds = fg_inds[np.random.choice(
            fg_inds_length, size=num_fgs, replace=fg_inds_length < num_fgs)]
        num_bgs = num_proposal - num_fgs
        bg_inds = bg_inds[np.random.choice(
            bg_inds_length, size=num_bgs, replace=bg_inds_length < num_bgs)]
